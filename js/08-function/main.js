...
const n2 = 2;
const n3 = 3;
const result2 = add(n1, 2, n3); // 2는 매개변수가 아님.

console.log(`result1 = ${result1}`);
console.log(`result2 = ${result2}`);
//매개변수 : 외부에서 들어온 데이터를 저장한 변수
// 인자 : 외부에서 전달한 데이터(값, 변수, 함수)

//호이스팅: 끌어올리다
//함수 선언식 정의 방법은 호이스팅이 된다
function add(number1, number2, number3) {
  return number1 + number2 + number3;
}
...
console.log(`sum(1, 2)= ${sub(1, 2)}`); // -1

const multi = (number1, number2) => {
  return number1 * number2;
};

//매개변수와 반환값으로 구분한 숨수 정의
//매개변수 X, 반환값 X => 활용도가 낮음
////"Hello World" 출력 함수
function sayHello() {
 ...
 * 1. function 키워드를 지운다
 * 2. 함수명을 지운다
 * 3. 매개변수와 함수 몸체(코드블록)를 => 연결한다
 * 4. 변수를 선언하고 변수에 화살표 함수를 할당 한다.
 */

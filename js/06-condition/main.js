...
if(조건식2) {
  ...
  else 아래에는 또 다른 else if나 else가 존재 할 수 없다
} 
---
if(조건식) {
} else {
...
let answr = number > 0 ? "양수" : "음수";
console.log(`answer : ${answr}`);

console.log(`answer : ${number > 0 ? "양수" : "음수"}`);
result = number > 0 ? "양수" : "음수";

/*
만약에 변수 number2 가 0 보다 크면, "양수" 출력
그런데 만약에 변수 number2가 0 보다 작으면, "음수" 출력
그런데 모두 아니라면, "0" 출력
...
console.log(number2 > 0 ? "양수" : number < 0 ? "음수" : "0");
...
// 사용자의 로그인 여부
let isLoggined = true;
const user = isLoggined === true ? "회원 사용자" : "비 회원 사용자 화면";

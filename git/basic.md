...
# 파일의 3가지 상태
 워킹 디렉토리 ─── git add ───> 스테이징 에어리어 ─── git commit ───> 레포지토리
...
### **git commit -m "메시지"**
  - 스테이징 영역에 있는 파일만 커밋.
  - 아직 git add 하지 않은 변경은 커밋에 포함되지 않음.

### **git commit -am "메시지"**
  - `-a` = **자동으로 수정(tracked)된 파일을 add 후 commit**
  - 즉, 이미 Git이 추적 중인 (tracked) 파일이 수정되면, 굳이 `git add` 하지 않아도 바로 커밋됨.
  - 하지만! **새로 만든 파일(한번도 add 한적 없는 untracked 파일)**은 포함 안됨
...
## 명령어
**터미널에서 쓰는 . (점)은 **현재 디렉토리(current directory)**
#### 지역 저장소 - 원격 저장소 연결 : 
...
### revert 옵션
1번쨰 커밋으로 돌아갈때 1번쨰 커밋만을 지정해서 그것을 취소 하고 revert 커밋을 남긴다

좋은점 : 기존 커밋이 유지된다. 

## git reset은 어떤 명령어인가요?

git reset은 과거 커밋 내역의 특정 커밋 위치로 돌아가는 명령어 이다. 해당 커밋의 hash값으로 git reset <해쉬값> 명령어를 입력하여 돌아간다. 
  - 해당 커밋 해쉬값의 커밋이 되었던 시점으로 돌아간다 

## git reset의 3가지 옵션은 무엇이고, 각각의 의미는 무엇인가요?
  - git reset --soft : 커밋만 취소한다. 스테이징 영역에는 유지
  - git reset --mixed : 커밋과 스테이징 영역 내용을 취소한다. 작업 트리 영역에는 유지
  - git reset --hard : 커밋취소, 스테이징 영역 내용 취소, 작업트리 영역 내용을 모두 취소 한다 

...
## 높이가 없으면 안의 컨텐츠에 의해서 높이가 결정된다. 그래서 텍스트 크기가 위아래 태그마다 다를경우 영역 height가 달라진다.

# Gap

gap: 플렉스/그리드 컨테이너 안에서 **형제 아이템 사이의 간격**을 전용으로 만들어 줘.
→ .nav-menu ul { display:flex; gap:16px; } ⇒ li와 li 사이가 벌어짐.

컨테이너에 padding: 컨테이너 “테두리와 첫/마지막 아이템” 사이의 안쪽 여백만 늘어남. 형제 간 간격은 그대로.
→ ul 안쪽 전체가 오른쪽/왼쪽으로만 멀어질 뿐, li들끼리는 붙어 있음.

컨테이너에 margin: 컨테이너 바깥 공간(부모와의 거리)을 조절. 아이템 간 간격과 무관.

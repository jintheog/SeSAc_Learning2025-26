...
```css
#parent > .child {
  /*id가 parent인 요소안의 class가 child인 요소들을 선택*/
  color: violet;
}
```
...
**기본값(static, float 없음)**이면 문서 순서대로 배치된다.

블록 요소: 위→아래

인라인 요소: 왼→오

부모 안에서도 동일한 규칙이 부모의 내부에서 적용된다.

position / float / flex / grid / writing-mode를 쓰면 이 기본 흐름이 바뀐다.
